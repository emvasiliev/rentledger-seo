...
export default function GuidesIndexPage() {
  return (
    <div className="mx-auto max-w-5xl px-4 py-12">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
        {PROVINCES.map((province) => (
          <a
            ...
            className="rounded-xl border border-gray-200 p-5 hover:border-[hsl(152_60%_60%)] hover:bg-[hsl(152_60%_96%)] transition-colors"
          >
            <div className="font-bold text-gray-900 text-lg">{province.name}</div>
            <div className="mt-1 text-sm text-gray-500">
              {province.majorCities.slice(0, 2).join(", ")}
            </div>
            <div className="mt-3 text-xs text-[hsl(152_60%_36%)] font-medium">
              {51} US state guides →
            </div>
          </a>
        ))}
      ...
      <div className="mt-16 rounded-xl bg-[hsl(152_60%_96%)] p-6 text-center">
